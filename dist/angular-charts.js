     ...
     */
      function pieChart() {
        ...
        path.append('path').style('fill', function (d, i) {
          return getColor(i);
        }).style('stroke', config.stroke).style('stroke-width', config.strokeWidth).transition().ease('linear').duration(config.isAnimate ? 500 : 0).attrTween('d', tweenPie).attr('class', 'arc').each('end', function () {
